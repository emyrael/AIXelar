      ...
      <div className="container relative z-10 mx-auto px-6">
        ...
        <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-2">
          {products.map((product, index) => (
            <article
              key={product.name}
              className="group gradient-border relative overflow-hidden rounded-2xl bg-card/85 p-6 shadow-soft backdrop-blur transition-all duration-500 hover:-translate-y-1 hover:shadow-glow"
              ...
              <div className="relative z-10">
                <div className="mb-7 flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="flex h-16 w-16 items-center justify-center rounded-xl border border-border/70 bg-background p-2 shadow-soft">
                      <img src={product.logo} alt={`${product.name} logo`} className="max-h-full max-w-full object-contain" />
                    </div>
                    <div>
                      <p className="text-sm font-semibold uppercase tracking-wider text-accent">{product.eyebrow}</p>
                      <h3 className="text-2xl font-bold text-foreground">{product.name}</h3>
                    </div>
                  </div>
                  <span className="shrink-0 rounded-full border border-accent/25 bg-accent/10 px-3 py-1 text-xs font-semibold text-accent">
                    {product.status}
                  </span>
                </div>
